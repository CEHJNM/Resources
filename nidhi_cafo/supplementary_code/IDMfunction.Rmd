...
make_grid <- function(region_counties, crs, grid_res) {
  ...
  region_grid <- Region |>
    st_bbox() |>
    st_make_grid(cellsize = grid_res)
  region_grid |>
    st_intersection(Region) -> region_grid
}

us_grid <- make_grid(us_counties_contiguous, crs = "EPSG:5070", grid_res = 10000)
...
g2 <- ggplot() +
  ...
  scale_color_manual(values = c("Feedlots" = "white")) +
  theme_minimal() +
