...
```{r}
path <- "~/Downloads/Summer 2025 Project/data/NASSAgCensusDownload2022_175maps_Links.xlsx"
sheet_names <- excel_sheets(path)
data_farms <- read_excel("data/NASSAgCensusDownload2022_175maps_Links.xlsx", sheet = "Farms")
...
```{r}
hogs_data <- read.csv("data/HogsInventory2022AgCensus.csv")
cattle_data <- read.csv("data/CattleInventory2022AgCensus.csv")
goats_data <- read.csv("data/GoatsInventory2022AgCensus.csv")
sheep_data <- read.csv("data/SheepInventory2022AgCensus.csv")
...
```{r}
rucc <- read_excel("data/Ruralurbancontinuumcodes2023.xlsx")
...
```{r}
land_cover <- rast("data/Annual_NLCD_LndCov_2023_CU_C1V0.tif")
plot(land_cover)
...
```{r}
naics <- read.csv("data/NAICS112.csv")
...
## CO
```{r}
co_pollutant <- read.csv("data/CO_pollutant.csv")
...
```{r}
toxic_releases <- read.csv("data/mn_toxic_releases.csv")
...
```{r}
library("ncdf4")
methane <- nc_open("data/Gridded_GHGI_Methane_v2_2018.nc")
print(methane)

methane <- raster("data/Gridded_GHGI_Methane_v2_2018.nc", varname = "emi_ch4_1A_Combustion_Mobile")
...
# Compiled CAFO Data
```{r}
cafo_compiled <- st_read("data/cafo_state_subset_with_coords/cafo_state_subset.shp")
...
## Louisiana Clean-up
```{r}
la_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Louisiana.csv")
...
## New York Clean-up
```{r}
ny_cafos <- st_read("data/CAFOs to be compiled/CAFOs in New York/CAFOs in New York.shp")
...
## Alabama Clean-up
```{r}
al_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Alabama.csv")
...
## Arkansas Clean-up
```{r}
ar_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Arkansas.csv")
...
## Colorado Clean-up
```{r}
co_cafos <- st_read("data/CAFOs to be compiled/CAFOS in Colorado/CAFOS in Colorado.shp")
...
## Florida Clean-up
```{r}
fl_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Florida.csv")
...
## Georgia Clean-up
```{r}
ga_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Georgia.csv")
...
## Texas Clean-up
```{r}
tx_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Texas.csv")
...
```{r}
ms_cafos <- read.csv("data/CAFOs to be compiled/CAFOs in Mississippi.csv")
...
## Wisconsin Clean-up
```{r}
wi_cafos <- read_excel("data/CAFOs to be compiled/CAFOs in Wisconsin.xlsx")
...
## Minnesota CAFOs
```{r}
mn_cafos <- read.csv("data/CAFOs to be compiled/MN_CAFOs.csv")
...
## Nebraska CAFOs
```{r}
ne_cafos <- st_read("data/CAFOs to be compiled/illinois_nebraska_cafo/Nebraska_CAFO.shp")
names(ne_cafos)
...
## Illinois CAFOs
```{r}
il_cafos <- st_read("data/CAFOs to be compiled/illinois_nebraska_cafo/Illinois_CAFO.shp")
names(il_cafos)
...
## Missouri CAFOs
```{r}
mo_cafos <- st_read("data/CAFOs to be compiled/MO_NPDES_Animal_Feeding_Operations/MO_NPDES_Animal_Feeding_Operations.shp")
...
## Washington CAFOs
```{r}
wa_cafos <- st_read("data/CAFOs to be compiled/WA_Dairies_2019_OCIO_-7324419961240643796/WADairies.shp")
...
## California CAFOs
```{r}
ca_cafos <- st_read("data/CAFOS to be compiled/CAFO_California/animal_type.shp")
names(ca_cafos)
...
# CAFO Density EPA
```{r, fig.align="center", echo = FALSE, fig.width = 14}
epa_cafo_density <- st_read("data/CAFO_Density/CAFOs_per_County.shp")
...
combined_cafos <- subset(combined_cafos, select = c(state_name, state, county, fips, facility_n, geometry))
st_write(combined_cafos, "combined_cafos_final.shp")
combined_cafos <- st_read("data/combined_cafos/combined_cafos_final.shp")
ggplot() +
